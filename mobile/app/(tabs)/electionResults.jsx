...
export default function ElectionResultsScreen() {
  ...
  useEffect(() => {
    if (params.electionId) {
      setSelectedElectionId(params.electionId);
    }
  }, [params.electionId]);
  ...
  const fetchResults = async () => {
    ...
    setLoading(true);
    try {
      const data = await apiService.getElectionResults(token, selectedElectionId);
  ...
  if (!election || !results || results.length === 0) {
    return (
      <View
        style={{
          flex: 1,
      ...
      >
        ...
        >
          No live results available for this election yet.
        </Text>
      </View>
    );
  }
